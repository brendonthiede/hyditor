...
- ✅ Full Preview fixed: Jekyll is spawned via the platform-appropriate shell (`bash -l -c` on Linux/macOS, `cmd.exe /C` on Windows) to pick up rbenv/rvm/system PATH; `bundle install` is run automatically before `bundle exec jekyll serve`; stderr is forwarded to the terminal; early process exit is detected and reported with an actionable message; `--baseurl ""` added to prevent URL routing issues; on Windows, `taskkill /F /T` is used to kill the full process tree when stopping the preview
...
- ✅ Whitespace-only change detection: files whose working-tree diff (index → workdir) is entirely whitespace are marked `whitespace_only` in the backend; they are hidden from the changed-files list by default and cannot be published; a "Show whitespace (N)" toggle reveals them (greyed out) for reference; the backend `git_stage` command also rejects any staging attempt for whitespace-only files as a safety net
- ✅ File tree filtering: a text input filters visible files by name (matching files and their ancestor directories are shown, all directories auto-expand while a query is active); a toggle button (`🔽` / `👁`) hides or reveals Jekyll-irrelevant directories (`node_modules`, `vendor`, `.bundle`, `_site`, `.sass-cache`, `.jekyll-cache`, `.jekyll-metadata`, `_includes`, `_layouts`, `_sass`) and non-text binary files (fonts, archives, etc.) — enabled by default with a "show all" notice showing the count of hidden items; image files are always visible so they can be referenced in posts; under the `assets/` folder, `js/`, `css/`, and `scss/` subdirectories are hidden while other content (e.g. images) remains visible
...
- ✅ Frontend dependency security maintenance: npm `overrides` now pins `cookie` to `0.7.2` to resolve the current `npm audit` advisory affecting the SvelteKit transitive dependency chain
- ✅ Jekyll baseurl launch fix: Full Preview now writes a temporary config override file in Hyditor's app cache directory (outside cloned repositories) and passes it via `--config` (instead of passing empty `--baseurl` on the command line), avoiding PowerShell/Jekyll 3 argument parsing issues on Windows; temporary config files are cleaned up when preview stops and stale leftovers are removed at next startup; diagnostics now include the exact temp config path used for the current launch
...
- ✅ AI Assistant (Google Gemini): a new **AI** blade tab in the left sidebar lets users chat with Google Gemini about their Jekyll site; the assistant automatically gathers text file contents from the repo as context (up to 500 KB, toggleable via the 📁 button); users configure their Gemini API key via the ⚙ settings panel (stored with 0600 permissions in the app data directory); supports multi-turn conversations with conversation history; selectable model (gemini-2.5-flash default, plus gemini-2.0-flash-lite, gemini-2.0-flash, gemini-1.5-flash, gemini-1.5-pro); backend `ai::gemini` Rust module handles API communication; frontend `AiPanel` component with chat UI, API key management, model selector, and repo context toggle
- ✅ AI file modifications: the AI assistant can directly apply file changes — when the AI suggests modifications it outputs structured `file:path` code blocks; each block renders with **Apply** (write to disk) and **Apply & Open** (write and open in editor) buttons; the currently open file's path and content are sent as context so the AI knows what you're editing; applied edits update the editor in real-time if the modified file is currently open
...
- ✅ AI prompt meta-analysis and versioning: built-in template prompts can now be edited (✎ button), reset to defaults (↺ button), and have their version history browsed (🕑 button); all prompt changes — manual edits, AI-suggested improvements, and reverts — are tracked as versioned snapshots with timestamps and change notes; the transcript stats panel shows an "✨ Analyze" button (requires 2+ uses) that sends the template prompt, usage statistics, and sample conversations to Gemini for analysis; Gemini returns an improved prompt in a structured format that can be applied with a single click ("Apply Suggested Prompt"); custom templates also support version history and revert; overrides for built-in templates are stored separately in localStorage so the originals are always recoverable
